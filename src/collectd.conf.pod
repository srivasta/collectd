...
=head2 Plugin C<gcp_metadata>

The I<gcp_metadata> plugin retrieves and parses metadata labels from
the I<GCP Metadata> server for the GCE instance the plugin is running on,
and adds specific metadata key value pairs to the collectd internal
metadata list, where it can be acted upon by a I<write> plugin.

Synopsis:

 <Plugin gcp_metadata>
   ExtraMetricFields "goobuntu-provisioning-user"
 </Plugin>

The list of Metadata keys the plugin pays attention to s specified in
the configuration variable B<ExtraMetricFields>, which should contain
a comma separated list of Metadata keys. If there is no such
configuration parameter, the plugin does nothing, and disables
itself. Otherwise it searches through the set of metadata keys, and
collects the specified key and value pairs, and adds them to the
B<collectd> internal metadata list. If none of the keys are found on
the metadata server list, the I<gcp_metadata> plugin takes no action.
